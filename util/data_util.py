...
def get_generic_csv_data(symbol: str, interval: str, start_time: datetime.datetime = None,
    ...
    if os.path.exists(path):
        ...
        # 最早数据记录
        earliest_record_time = data[0][0]
        # 最新的数据记录
        latest_record_time = data[len(data)[0]]
...
def get_k_lines(symbol: str, interval: str, start_time: datetime.datetime = None,
                end_time: datetime.datetime = datetime.datetime.now()):
    ...
    while True:
        # 获取数据
        if stop_time.timestamp() >= start_time.timestamp():
...
if __name__ == '__main__':
    data = get_generic_csv_data(symbol='OPUSDT', interval="1d")
    print(data)
